...
<script src="{{asset('Agent/assets/js/pace.min.js')}}"></script> --}}
   ...
   <style>
    /* ==================== REAL WHATSAPP WEB UI - COMPLETE CSS ==================== */
                ...
                <div class="chat-sidebar-header">
                    <div class="d-flex align-items-center">
                        <div class="chat-user-online">
                            @php
                                if (!empty(Auth::guard('agent')->user()->image)) {
                                    $selfimage = asset(Auth::guard('agent')->user()->image);
                                } else {
                        ...
                        <div class="flex-grow-1 ms-2">
                            <p class="mb-0">{{ session('agent_alise_name') }}</p>
                        </div>
                        <button type="button" onclick="window.location.href='{{ route('agent_dashboard') }}'"
                                        ...
                                        @foreach ($filtered_groups as $group)
                                            @php
                                                ...
                                                $message = App\Models\ChMessage::where('group_id', $group->id)
                                                    ...
                                                    $check_seen = App\Models\ChMessage::where('group_id', $group->id)
                                                    ->get();
                                                    $unreadCount = 0;
                                                ...
                                                <a href="javascript:;"
                                                    ...
                                                    @else
                                                        <small class='bx bxs-circle me-1 chart-online'
                                                            style="color:orange"></small>Active {{ $formattedDiff ?? '' }}
                                                        ago
                                                    @endif
                                                </a>
...
<a href="javascript:;" class="list-inline-item d-flex align-items-center text-secondary search-user"
   data-user-id="{{ $user->id }}">
   <i class='bx bx-search me-1'></i>Find
</a>

<input type="text" id="search-user-input-{{ $user->id }}"
   class="form-control search-user-input"
   data-user-id="{{ $user->id }}"
   placeholder="Search messages..."
   style="display:none; margin-left:10px; width:90%; margin-top:6px; width:90%;">
                            ...
                            <div class="d-flex">
                                <div class="flex-grow-1 ms-2">
                                    @php
                                        ...
                                        $group_use_alise = $group_user->alise_name ?? 'admin';
                                    @endphp
                                    <p class="mb-0 chat-time">{{ $group_use_alise }},
                                        {{ $message->created_at->format('h:i A') }}</p>
                                    {{-- @if (!empty($message->body))
                                                                    <p class="chat-left-msg">{{ $message->body }}</p>
                                                                @endif --}}
                                                        ...
                                                        <div class="col-sm-6">
                                                            <div class="input-group-prepend">
                                                                <input type="checkbox"
                                                                    ...
                                                                    aria-label="Checkbox for following text input"
                                                                    name="group_ids[]" value="{{ $forword_id->id }}">
                                                                <label class="form-check-label"
            ...
            <div class="flex-grow-1 pe-2">
                ...
                <div class="input-group mb-3">
                    <span class="input-group-text emoji-button" data-target="emoji-picker-{{ $group->id }}"><i
                            class='bx bx-smile'></i></span>
                    <textarea id="body-group-{{ $group->id }}" name="body"  class="form-control" style="height: 10px;" placeholder="Type a message"
                        oninput="autoResize(this)" onkeydown="checkEnter(event, '{{ $group->id }}', 'group')"></textarea>
                </div>
            </div>
                                        ...
                                        <div class="col-sm-6">
                                            <div class="input-group-prepend">
                                                ...
                                                @php
                                                    ...
                                                    if (!empty($alise->alise_name)) {
                                                        $name_alise = $alise->alise_name;
                                                    } else {
                                                        $name_alise = $user->name;
                                                    }
                                                @endphp
                                                <label class="form-check-label" for="checkbox-{{ $user->id }}"
                                                    style="padding:1px">{{ $name_alise }}</label>
                                            </div>
                                        </div><br><br>
                                    @endforeach
                                @endif
...
channel.bind('my-chat-event', function(data) {
    ...
    var toId = data.message.to_id; // <-- get receiver id
    ...
    // Append the message **only if you are the sender or receiver**
    if (isOwnMessage || isReceiver) {
        if (!groupId) {
            if (isReceiver) {
                $('#chat-content-' + fromId).append(newMessage);
            } else {
                $('#chat-content-' + toId).append(newMessage);
            }
            var chatContent = $('#chat-content-' + (isOwnMessage ? toId : fromId));
            chatContent.scrollTop(chatContent.prop("scrollHeight"));
        }
    }
...
function resetTimer() {
    ...
    inactivityTimer = setTimeout(logoutUser, 180000); // 1 minute (adjust as needed)
}

$(document).ready(function() {
    resetTimer(); // Start the timer when the document is ready

    // Listen for keypress and mousemove events to reset the timer
    $(document).on('keypress mousemove', function() {
        resetTimer();
    });
});

// Function to update the count of open tabs in local storage
function updateTabCount(count) {
    localStorage.setItem('openTabsCount', count.toString());
}

// Check if this tab is the first one open and initialize the count
if (!localStorage.getItem('openTabsCount')) {
    updateTabCount(1); // Start with 1 tab open
} else {
    let count = parseInt(localStorage.getItem('openTabsCount'));
    updateTabCount(count + 1); // Increment count for each additional tab open
}

// Decrease the count of open tabs when a tab is closed
window.addEventListener('beforeunload', function() {
    let count = parseInt(localStorage.getItem('openTabsCount'));
    if (count > 0) {
        updateTabCount(count - 1);
    }
});

// Check if this is the last tab being closed
window.addEventListener('unload', function() {
    let count = parseInt(localStorage.getItem('openTabsCount'));
    if (count === 1) {
		console.log('calling logout user'); // Log message for tracking
		logoutUser();
    }
});

function logoutUser() {
	console.log('enter in logout user'); // Log message for tracking
	var logoutUrl = $('#routeUrls').data('logout');
	var loginUrl = $('#routeUrls').data('login');
    ...
    <script>
        // Find click → search box open for that user
$(document).on('click', '.search-user', function() {
    const userId = $(this).data('user-id');
    $('#search-user-input-' + userId).slideToggle(200).focus();
});

// Typing → filter that user's messages
$(document).on('input', '.search-user-input', function() {
    const userId = $(this).data('user-id');
    const query = $(this).val().toLowerCase();

    $('#chat-' + userId + ' .chat-message').each(function() {
        const msgText = $(this).text().toLowerCase();
        $(this).toggle(msgText.includes(query));
    });
});

    </script>
    <script>
        // जब backend से notification मिले → count बढ़ाओ
function increaseNoti(userId) {
  const el = document.getElementById("chat-noti-count-" + userId);
  if (!el) return;
  let count = parseInt(el.innerText);
  el.innerText = count + 1;
}

// Bell click → वही user chat open करो
$(document).on("click", ".notification-bell", function () {
  ...
  // scroll bottom
  const box = document.getElementById("chat-content-" + id);
  if (box) box.scrollTop = box.scrollHeight;

  // notification reset
  const badge = document.getElementById("chat-noti-count-" + id);
