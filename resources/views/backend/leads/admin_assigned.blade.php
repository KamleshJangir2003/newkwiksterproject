                                ...
                                <div class="flex-grow-1 ms-3">
                                    <h6 class="fs-14 mb-1">{{ count($excelData) }} Data to
                                        <small class="badge bg-primary-subtle text-primary"> 
                                            @php
                                                $firstUserData = $excelData->first()->adminfwd->user ?? null;
                                                $userName = optional($firstUserData)->name ?? 'N/A';
                                            @endphp {{ $userName }} (Manager)
                                        </small>
                                    </h6>
                                    <p class="text-muted mb-0">
                                        {{ optional($excelData->first()->adminfwd)->created_at
                                            ? $excelData->first()->adminfwd->created_at->format('H:i:s') .
                                                ' - ' .
                                                $excelData->first()->adminfwd->created_at->format('d/m/y')
                                            : 'N/A' }}
                                    </p>
                                </div>
                            </a>
                        ...
                        <div class="collapse border-top border-top-dashed" id="leadDiscovered{{ $batchId }}">
                            <div class="card-footer hstack gap-2">
                                <a class="btn btn-primary btn-sm w-100"
                                    href="{{ route('admin.view.assigned.leads', encrypt($batchId)) }}"><i
                                        class="ri-question-answer-line align-bottom me-1"></i>
                                    View Data</a>
                                    <a class="btn btn-danger btn-sm w-100"
                                    href="{{ route('admin.delete.assigned.leads', encrypt($batchId)) }}"  onclick="return confirm('Are you sure ?');"><i
                                        class="ri-question-answer-line align-bottom me-1"></i>
                                    Delete Data</a>
                            </div>
                        </div>
