    ...
    <div class="row">
        <div class="col-12">
            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                <h4 class="mb-sm-0">Client Forms</h4>

                <div class="page-title-right">
                    <ol class="breadcrumb m-0">
                        <li class="breadcrumb-item"><a href="javascript: void(0);">CRM</a></li>
                        <li class="breadcrumb-item active">Client Forms</li>
                    </ol>
                </div>
    ...
    <form action="{{ route('manager.fwd.data.agent') }}" method="post">
                            ...
                            <div class="col-sm-auto ms-auto">
                                <div class="hstack gap-2">
                                    ...
                                    <button type="button" class="btn btn-primary" data-bs-toggle="modal"
                                        data-bs-target="#addClientForm"><i class='bx bx-plus-medical me-1'></i>
                                        Add</button>
                                ...
                                <table class="table align-middle" id="customerTable">
                                    <thead class="table-light">
                                        <tr>
                                            ...
                                            <th class="sort" data-sort="date">Upload Date</th>
                                            <th class="sort" data-sort="name">India Agent Name</th>
                                            <th class="sort" data-sort="name">Customer Name</th>
                                            <th class="sort" data-sort="phone">Phone</th>
                                            <th class="sort" data-sort="company_name">Company Name</th>
                                            <th class="sort" data-sort="leads_score">Pdf</th>
                                            <th class="sort" data-sort="name">Step</th>
                                            <th class="sort" data-sort="location">Status</th>
                                            <th class="sort" data-sort="name">Priority</th>
                                            <th class="sort" data-sort="name">S/M/H/D</th>
                                    ...
                                    <tbody class="list form-check-all">
                                        ...
                                        @foreach ($client_form as $data)
                                            <tr class="@if ($data->priority == '1') pulse @endif" data-date="{{ \Carbon\Carbon::parse($data->created_at)->format('d-m-y') }}">
                                                ...
                                                <td class="name">{{ $data->india_agent_name }}</td>
                                                <td class="name">{{ $data->customer_name }}</td>
        ...
        @endif
                                                ...
                                                <td style="width: 150px;" class="dropdown">
                                                    ...
                                                    <div class="progress animated-progress custom-progress mb-4"
                                                        ...
                                                        <div class="progress-bar bg-{{ $data->step === 'Not Connected' ? 'primary' : ($data->step === 'Contacted' ? 'warning' : ($data->step === 'Quotation Sent' ? 'success' : ($data->step === 'Closing' ? 'danger' : 'info'))) }}"
                                                            role="progressbar"
                                                            style="width: {{ $data->step_value ?? 100 }}%"
                                                ...
                                                <td>
                                                    <span
                                                        class="badge 
                                                     @if ($data->priority === 1) bg-danger
                                                     @else
                                                         bg-success-subtle text-success @endif">
                                                        {{ $data->priority ? 'High Priority' : 'Normal' }}
                                                    </span>
                                                </td>
                                                <td>
                                                    @php
                                                ...
                                                <td>
                                                    @if (auth()->user()->designation == 'Admin')
                                                        <a href="{{ route('admin.delete.client.form', $data->id) }}"
                                                    ...
                                                    @else
                                                       <div class="row">
                                                       <div class="col-4">
                                                            <button class="btn btn-soft-secondary btn-sm data-select1"
                                                       ...
                                                       <div class="col-6">
                                                            <button class="btn btn-soft-secondary btn-sm addClientPdf"
                                                            type="button" data-id="{{ $data->customer_id }}"
                                                            data-clientdataid="{{ $data->id }}">
                                                            PDF
                                                        </button>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="row">
                        ...
                        <div class="row border-bottom border-top border-primary">
                            <div class="col-lg-12 py-2">
                                    <b>Commodities</b><p id="commodities"></p>
                                    </div>
                               </div>
                        <div class="col-lg-12 border-end border-primary">
...
@section('js')
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.data-select1').on('click', function() {
            ...
            function updateModal(data) {
                ...
                var formattedDOB = dateParts[1] + '-' + dateParts[2] + '-' + dateParts[0];
    ...
    {{-- Fetch Client Form Data --}}
    <script>
        $(document).ready(function() {
            $('.fetch-data').on('change', function() {
                var dataId = $(this).val();
                $.ajax({
                    url: '/get/agent/customer/data/' +
                        dataId, // Replace with your actual API endpoint
                    type: 'GET',
                    success: function(data) {
                        $('.clientFormAgentId').val(data.user.id);
                        $('.india_agent_name').val(data.user.name);
                        $('.customer_name').val(data.excelData.company_rep1);
                        $('.phone').val(data.excelData.phone);
                        $('.client_company_name').val(data.excelData.company_name);
                    },
                    error: function(error) {
                        console.log(error);
                    }
